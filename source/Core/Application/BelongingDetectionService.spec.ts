...
import { MuTagSignal, MuTagMonitor } from '../Ports/MuTagMonitor';
import { MuTagRepositoryLocal } from '../Ports/MuTagRepositoryLocal';
import { Observable, Subscriber } from 'rxjs';
import BelongingDetectionService from './BelongingDetectionService';

let onMuTagDetectionSubscriber: Subscriber<MuTagSignal[]>;
let onMuTagRegionExitSubscriber: Subscriber<BeaconID[]>;
const MuTagMonitorMock  = jest.fn<MuTagMonitor, any>((): MuTagMonitor => ({
    onMuTagDetection: new Observable<MuTagSignal[]>((subscriber): void => {
        onMuTagDetectionSubscriber = subscriber;
    }),
    onMuTagRegionExit: new Observable<BeaconID[]>((subscriber): void => {
...
const belongingDetectionService
    = new BelongingDetectionService(muTagMonitorMock, muTagRepoLocalMock);
const muTagData = {
    _uid: 'UUID01',
...
describe('last seen status of belongings continuously updates', (): void => {
    ...
    describe('app device is in range of a belonging beacon', (): void => {
        ...
        //
        const detectedMuTag: MuTagSignal[] = [{
    ...
    describe('app device has exited belonging beacon region outside of assigned Safe Zone', (): void => {
        ...
        //
        const exitedMuTag: BeaconID[] = [BeaconID.create('0')];
