...
test("successfully gets added Mu tag", async (): Promise<void> => {
    ...
    await expect(
        muTagRepoLocalImpl.getByBeaconID(muTagBeaconID)
    ).resolves.toEqual(muTag);
});

test("successfully removes Mu tag", async (): Promise<void> => {
    (database.remove as jest.Mock).mockResolvedValueOnce(undefined);
    expect.assertions(1);
    await expect(muTagRepoLocalImpl.removeByUID(muTagUID)).resolves.toEqual(
        undefined
    );
});
...
test("failed to get Mu tag that does not exist", async (): Promise<void> => {
    (database.get as jest.Mock).mockResolvedValueOnce(null);
    expect.assertions(1);
    await expect(muTagRepoLocalImpl.getByUID(muTagUID)).rejects.toEqual(
        new MuTagDoesNotExist(muTagUID)
    );
});
